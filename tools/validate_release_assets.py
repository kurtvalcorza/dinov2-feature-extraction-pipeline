...
EXPECTED_MODEL_ID = "timm/vit_small_patch14_dinov2.lvd142m"
...
EXPECTED_OUTPUTS = ('outputs/dinov2_feature_extraction_result.json',)
# Profile-specific code the notebook must exercise through the repository public API.
CODE_MARKERS = (
    'from dinov2_feature_extraction_pipeline import EMBED_DIM, MAX_BATCH, MAX_IMAGE_SIDE, '
    'NORMALIZED, POOLING',
    'from dinov2_feature_extraction_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'DINOv2FeatureExtractionPipeline, stage_missing_files, verify_snapshot',
    ...
    "contract = {'EMBED_DIM': EMBED_DIM, 'POOLING': POOLING, 'NORMALIZED': NORMALIZED, "
    "'unit': 'one vector per image'}",
    ...
    "{'id': name, 'pixel_sha256': digests[name], 'width': images[name].width, "
    "'height': images[name].height, 'vector': result['embeddings'][index]}",
...
MARKDOWN_MARKERS = (
    ...
    '`POOLING` (`"cls"`: the class token after the final LayerNorm',
    'one vector **per image**, not per patch or per token',
    'there is no missing-data concept',
    '**no metric helper and reports no performance measure**',
    'Evaluating these features requires a downstream labelled task',
    '**qualitative check only**',
    'the pipeline deliberately ships none',
    'image classification, object detection, segmentation, captioning, image-text comparison',
    '`../docs/WEIGHTS.md`',
)
